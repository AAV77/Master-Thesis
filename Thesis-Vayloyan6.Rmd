...
# Loading Raw Data
d.ED_GDP <- read.xlsx(path)

# Adding World Bank Codes

d.ED_GDP$Country <- countrycode(d.ED_GDP$Country, "country.name", "iso3c")
...
d.ED_GDP$'2019' <- as.numeric(d.ED_GDP$'2019')
d.ED_GDP$'2020' <- as.numeric(d.ED_GDP$'2020')
...
run_mcar_tests1(d.Adoption, d.Inflation, d.GDS, d.GDP, d.RR, d.ED_GDP)
```
...
-   Average for all observations, divided by year, imputed for 1 country value in *d.RR*, as there were no values for any year for that country United Arab Emirates. No other imputations had to be applied.

-   No imputations required for *d.CC.*

-   No imputations required for *d.ED_GDP.*
...
d.ED_GDP_imp <- fill_na_with_nearest_avg(d.ED_GDP)
...
Figure \@ref(fig:bw-Corruption) shows that while there are no extreme outliers in *d.Corruption*, there is a right skew in the data. This is further confirmed by Figure \@ref(fig:hist-Corruption), which shows that the distribution is right skewed, with a small peak in the right skew.
...
```{r bw-ED-GDP, echo = F, fig.width=4, fig.height=3, fig.cap="Box and Whisker Plots for d.ED_GDP"}
plot_summary_boxplots(d.ED_GDP_imp[,-1], title = NULL)
```

\FloatBarrier

```{r hist-ED-GDP, echo = F, message = F, fig.width=4, fig.height=3, fig.cap="Histograms for d.ED_GDP"}
plot_summary_histograms(d.ED_GDP_imp[,-1],20)
```
...
dataset_imp_list <- c( "d.CC_imp","d.Corruption_imp","d.ED_GDP_imp", "d.GDP_imp", "d.GDS_imp", "d.Inflation_imp","d.RR_imp", "d.Adoption_imp")
...
d.panel<-d.panel %>% 
  ...
  left_join(d.ED_GDP_imp_long, by= c( "Country","Year")) 
...
kable(preProcess_model_presentation, caption = "Yeo Johnson Transformation Lambda (rounded)", format = "latex", booktabs = TRUE, escape = FALSE) %>%
...
```{r md-ED-GDP, echo=F, fig.cap="Missing Pattern d.ED_GDP", fig.width=5, fig.height=4, message = F, results = "hide"}
md.pattern(d.ED_GDP_imp[,-1])
```
