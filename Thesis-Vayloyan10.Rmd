...
3.  Backing of the stablecoin by one or more cryptocurrency issued by the same entity as the stablecoin, which can be used to alter the price of the stablecoin, with the goal of maintaining a desired peg (algorithmic stablecoins).
...
```{=tex}
\begin{align*}  
Adoption_{it} = \beta_1 \cdot Inflation_{it} 
+ \beta_2 \cdot GDS_{it} 
+ \beta_3 \cdot GDP_{it} 
+ \beta_4 \cdot Corruption_{it} \\ 
+ \beta_5 \cdot RR_{it} 
+ \beta_6 \cdot CC_{it} 
+ \beta_7 \cdot ED_{it} + \alpha_i + \varepsilon_{it}
\end{align*}
```
The term $\varepsilon_{i,t}$ represents the error term for each country and time.

The term $\alpha_i$ represents country specific, time invariant effects.

Notice the loss of the intercept $\beta_0$ [[**explain why the intercept is lost**]{.underline}].
...
kable(alpha_wide, caption = "Model 3: Country Fixed Effects (Alpha Coefficients)", align = "l", row.names = F) %>% 
  kable_styling(latex_options = c("hold_position"))
```
...
Point 3: It would have been good to include more granular predictors. This was not feasible due to the lacking data size, using dimensional reduction techniques such as PCA, which would allow the inclusion of more variables.

Point 4: Barely any variation in the *d.CC* variable, while technically the coefficient can still be estimated, this is an issue for generalizability, particularily in the fixed effects model.
